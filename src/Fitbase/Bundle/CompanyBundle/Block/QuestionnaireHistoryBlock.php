...
class QuestionnaireHistoryBlock extends SecureBlockServiceAbstract
{
    ...
    public function setDefaultSettings(OptionsResolverInterface $resolver)
    {
        $resolver->setDefaults(array(
            'template' => 'Company/Block/QuestionnaireHistory.html.twig',
        ));
    }
    ...
    public function render(BlockContextInterface $blockContext, Response $response = null)
    {
        if (($user = $this->serviceUser->current()) and ($company = $user->getCompany())) {
            ...
            if (!($collection = $repositoryQuestionnaireCompany->findByCompany($company))) {
                $collection = array();
            }
